...
export const removeEmptyEntries = (data: CVFormValues): CVFormValues => ({
  ...data,
  workExperience:
    data.workExperience?.filter((entry) => !isWorkEntryEmpty(entry)) || [],
  education: data.education?.filter((entry) => !isEduEntryEmpty(entry)) || [],
  languages:
    data.languages?.filter((entry) => !isLanguageEntryEmpty(entry)) || [],
  skills: data.skills?.filter((entry) => !isSkillEntryEmpty(entry)) || [],
});
